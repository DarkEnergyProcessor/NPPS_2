...
foreach(npps_query("SELECT event_id, end_time FROM `event_list` WHERE end_time > $UNIX_TIMESTAMP AND start_time <= $UNIX_TIMESTAMP") as $event)
{
...
return [
	[
		...
		'member_category_list' => [['member_category' => 1, 'banner_list' => $banner_list], ['member_category' => 2, 'banner_list' => $banner_list]]
	],
	200
];
?>